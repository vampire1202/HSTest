...
{
	/// <summary>
	/// 数据访问类:GBT236152009_TensileZong
	/// </summary>
	public partial class GBT236152009_TensileZong
	{
		...
		public int GetMaxId()
		{
		return DbHelperOleDb.GetMaxID("ID", "tb_GBT236152009_TensileZong"); 
		}
		...
		public bool Exists(int ID)
		{
			...
			strSql.Append("select count(1) from tb_GBT236152009_TensileZong");
		...
		public bool Add(HR_Test.Model.GBT236152009_TensileZong model)
		{
			...
			parameters[21].Value = model.L0;
			parameters[22].Value = model.b2;
			parameters[23].Value = model.t;
			parameters[24].Value = model.S0;
			parameters[25].Value = model.Fmax;
			parameters[26].Value = model.T2;
			parameters[27].Value = model.Z;
			parameters[28].Value = model.E;
			parameters[29].Value = model.isFinish;
			parameters[30].Value = model.testDate;
			parameters[31].Value = model.condition;
			parameters[32].Value = model.controlmode;
			parameters[33].Value = model.isUseExtensometer;
            parameters[34].Value = model.isEffective;
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(Model.GBT236152009_TensileZong model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update tb_GBT236152009_TensileZong set ");
		...
		public bool Delete(int ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from tb_GBT236152009_TensileZong ");
			strSql.Append(" where ID=@ID");
			OleDbParameter[] parameters = {
					new OleDbParameter("@ID", OleDbType.Integer,4)
			};
		...
		public HR_Test.Model.GBT236152009_TensileZong GetModel(int ID)
		{
			...
			HR_Test.Model.GBT236152009_TensileZong model=new HR_Test.Model.GBT236152009_TensileZong();
		...
		/// 得到一个对象实体
		/// </summary>
		public HR_Test.Model.GBT236152009_TensileZong GetModel(string testSampleNo)
		{
        ...
        public DataSet GetNotOverlapList(string strWhere)
        {
            ...
            strSql.Append(" FROM tb_GBT236152009_TensileZong ");
            if (strWhere.Trim() != "")
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select testSampleNo as [试样编号],[L0] as [标距L0(mm)],round([Fmax],4) as [Fmax(kN)], round([T2],2) as [T2(MPa)], round([E],2) as [E(MPa)], round([Z],2) as [A(%)]");
            strSql.Append("  FROM tb_GBT236152009_TensileZong ");// ,FORMAT(testDate,'YYYY-MM-DD') as [试验日期]
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
        ...
        public DataSet GetFinishAvg(string strWhere)
        {
            ...
            strSql.Append("select Round(Avg([T2]),2) as [T2] ");
            strSql.Append(" FROM tb_GBT236152009_TensileZong ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select isEffective as [试验无效], testSampleNo as [试样编号],[L0] as [标距Lo(mm)],[b2] as [宽度b2(mm)],[t] as [厚度t(mm)]");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  testSampleNo as [试样编号],[L0] as [标距Lo(mm)],[b2] as [宽度b2(mm)],[t] as [厚度t(mm)]");
        ...
        public DataSet GetFinishReport(string strWhere,double maxValue)
        {
            ...
            strSql.Append(" round([T2],4) as [T2(MPa)]  FROM tb_GBT236152009_TensileZong ");// ,FORMAT(testDate,'YYYY-MM-DD') as [试验日期]
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
		...
		/// 得到一个对象实体
		/// </summary>
		public HR_Test.Model.GBT236152009_TensileZong DataRowToModel(DataRow row)
		{
			HR_Test.Model.GBT236152009_TensileZong model=new HR_Test.Model.GBT236152009_TensileZong();
			if (row != null)
			{
                ...
                model.b2 =Convert.ToDouble(  row["b2"].ToString());
                ...
                model.T2 = Convert.ToDouble( row["T2"].ToString());
                model.Z =Convert.ToDouble(  row["Z"].ToString());
                model.E =Convert.ToDouble(  row["E"].ToString());
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from tb_GBT236152009_TensileZong T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "tb_GBT236152009_TensileZong";
