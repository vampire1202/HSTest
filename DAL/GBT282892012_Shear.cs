...
{
	/// <summary>
	/// 数据访问类:GBT282892012_Shear
	/// </summary>
	public partial class GBT282892012_Shear
	{
		public GBT282892012_Shear()
		{}
		#region  BasicMethod
		...
		public int GetMaxId()
		{
		return DbHelperOleDb.GetMaxID("ID", "tb_GBT282892012_Shear"); 
		}
		...
		public bool Exists(int ID)
		{
			...
			strSql.Append("select count(1) from tb_GBT282892012_Shear");
        ...
        public DataSet GetFinishAvg(string strWhere)
        {
            ...
            strSql.Append("select Round(Avg([T]),2) as [T],Round(Avg([C1]),2) as [C1] ");
            strSql.Append(" FROM tb_GBT282892012_Shear ");
            if (strWhere.Trim() != "")
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select testSampleNo as [试样编号],[L] as [长度L(mm)],round([FTmax],4) as [FTmax(kN)], round([T],2) as [T(N/mm)], round([C1],2) as [C1(N/mm²)]");
            strSql.Append("  FROM tb_GBT282892012_Shear ");// ,FORMAT(testDate,'YYYY-MM-DD') as [试验日期]
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        public DataSet GetFinishReport(string selCol, string strWhere)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        public DataSet GetFinishListFind(string strWhere,double maxValue)
        {
            ...
            if (maxValue < 1000.0)
            {
                switch (_dotValue)
                {
                    case 2: strSql.Append(" FORMAT([FTmax],'0.00') as [FTmax(N)],");
                        break;
                    case 3: strSql.Append(" FORMAT([FTmax],'0.000') as [FTmax(N)],");
                        break;
                    case 4: strSql.Append(" FORMAT([FTmax],'0.0000') as [FTmax(N)],");
                        break;
                    case 5: strSql.Append(" FORMAT([FTmax],'0.00000') as [FTmax(N)],");
                        break;
                    default: strSql.Append(" FORMAT([FTmax],'0.00') as [FTmax(N)],");
                        break;
                }
            }
            if (maxValue >= 1000.0)
            {
                switch (_dotValue)
                {
                    case 2: strSql.Append(" FORMAT([FTmax]/1000.0,'0.00') as [FTmax(kN)],");
                        break;
                    case 3: strSql.Append(" FORMAT([FTmax]/1000.0,'0.000') as [FTmax(kN)],");
                        break;
                    case 4: strSql.Append(" FORMAT([FTmax]/1000.0,'0.0000') as [FTmax(kN)],");
                        break;
                    case 5: strSql.Append(" FORMAT([FTmax]/1000.0,'0.00000') as [FTmax(kN)],");
                        break;
                    default: strSql.Append(" FORMAT([FTmax]/1000.0,'0.00') as [FTmax(kN)],");
                        break;
                }
            }

            strSql.Append(" round([T],2) as [T(N/mm)], round([C1],2) as [C1(N/mm²)]  FROM tb_GBT282892012_Shear ");// ,FORMAT(testDate,'YYYY-MM-DD') as [试验日期]
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        public DataSet GetFTmax(string strWhere)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select Max([FTmax]) as [FTmax] ");
            strSql.Append("  FROM tb_GBT282892012_Shear ");// ,FORMAT(testDate,'YYYY-MM-DD') as [试验日期]
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
		...
		public bool Add(HR_Test.Model.GBT282892012_Shear model)
		{
			...
			OleDbParameter[] parameters = {
					...
					new OleDbParameter("@FTmax", OleDbType.Double),
					new OleDbParameter("@T", OleDbType.Double),
					new OleDbParameter("@C1", OleDbType.Double),
			...
			parameters[22].Value = model.FTmax;
			parameters[23].Value = model.T;
			parameters[24].Value = model.C1;
			parameters[25].Value = model.isFinish;
			parameters[26].Value = model.testDate;
			parameters[27].Value = model.condition;
			parameters[28].Value = model.controlmode;
			parameters[29].Value = model.isUseExtensometer;
            parameters[30].Value = model.isEffective;
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(Model.GBT282892012_Shear model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update tb_GBT282892012_Shear set ");
		...
		public bool Delete(int ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from tb_GBT282892012_Shear ");
			strSql.Append(" where ID=@ID");
			OleDbParameter[] parameters = {
					new OleDbParameter("@ID", OleDbType.Integer,4)
			};
		...
		public HR_Test.Model.GBT282892012_Shear GetModel(int ID)
		{
			...
			HR_Test.Model.GBT282892012_Shear model=new HR_Test.Model.GBT282892012_Shear();
        ...
        /// 得到一个对象实体
        /// </summary>
        public HR_Test.Model.GBT282892012_Shear GetModel(string testSampleNo)
        {
		...
		/// 得到一个对象实体
		/// </summary>
		public HR_Test.Model.GBT282892012_Shear DataRowToModel(DataRow row)
		{
			HR_Test.Model.GBT282892012_Shear model=new HR_Test.Model.GBT282892012_Shear();
			if (row != null)
			{
                ...
                model.FTmax = Convert.ToDouble( row["FTmax"].ToString());
                model.T = Convert.ToDouble( row["T"].ToString());
                model.C1 = Convert.ToDouble( row["C1"].ToString());
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from tb_GBT282892012_Shear T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "tb_GBT282892012_Shear";
