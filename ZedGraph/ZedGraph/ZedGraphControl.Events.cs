...
namespace ZedGraph
{
	...
	#region Mouse Events
		...
		public void ZedGraphControl_MouseDown( object sender, MouseEventArgs e )
		{
			_isPanning = false;
			_isZooming = false;
			_isEditing = false;
		...
		/// <param name="sender">A reference to the <see cref="ZedGraphControl" /></param>
		/// <param name="e">A <see cref="MouseEventArgs" /> instance</param>
		public void ZedGraphControl_MouseUp( object sender, MouseEventArgs e )
		{
		...
		/// </param>
		public void ZedGraphControl_MouseMove( object sender, MouseEventArgs e )
		{
			if ( _masterPane != null )
			{
				Point mousePt = new Point( e.X, e.Y );
				...
				if ( _isZooming )
					...
					HandlePointValues( mousePt,z.GraphPane.YAxis.Title.Text,z.GraphPane.XAxis.Title.Text);
				//Revision: JCarpenter 10/06
				else if ( _isSelecting )
					HandleZoomDrag( mousePt );
			}
		...
		private Point HandlePointValues( Point mousePt,string _y,string _x )
		{
			int iPt;
			GraphPane pane;
			object nearestObj;
						...
						else
						{
							...
							else
							{
								...
								else
								{
									...
									this.pointToolTip.SetToolTip( this, "(   " + _y +": "+ double.Parse(yStr).ToString("f3") + "   ,   "+_x +": "+ double.Parse(xStr).ToString("f3") + "   )" );
									//this.pointToolTip.SetToolTip( this,
	...
	#region Mouse Wheel Zoom Events
		...
		public void ZoomPane( GraphPane pane, double zoomFraction, PointF centerPt,
	...
	#region Zoom Events
		...
		public void HandleZoomFinish( object sender, MouseEventArgs e )
		{
