...
namespace ZedGraph
{
	...
	#region Rendering Methods
		...
		{
			_scale._minAuto = false;
            _scale._min = 0;
			_scale._maxAuto = true;
            _scale._majorStepAuto = false;
            _scale._minorStepAuto = false;
			_crossAuto = false;
			//_scale._magAuto = true;
			//this.numDecAuto = true;
			_scale._formatAuto = true;

            //int xmax = (int)_scale.Max;
            //_scale.Min = 0;
            //_scale.Max = (xmax / 10) * 10;
            //_scale.MajorStep = _scale.Max / 5;
            //_scale.BaseTic = 0;


            //_scale._majorStep = (_scale.Max - _scale.Min) / 5;
            //_scale._minorStep = _scale._majorStep / 5;

            //showCurveResultPanel.XAxis.Scale.MajorStep = (showCurveResultPanel.XAxis.Scale.Max - showCurveResultPanel.XAxis.Scale.Min) / 5;
            //showCurveResultPanel.XAxis.Scale.MinorStep = showCurveResultPanel.XAxis.Scale.MajorStep / 5;
            //showCurveResultPanel.XAxis.MajorGrid.IsVisible = true;
            //showCurveResultPanel.XAxis.MinorTic.IsInside = true;

            //showCurveResultPanel.YAxis.Scale.MajorStep = (showCurveResultPanel.YAxis.Scale.Max - showCurveResultPanel.YAxis.Scale.Min) / 5;
            //showCurveResultPanel.YAxis.Scale.MinorStep = showCurveResultPanel.YAxis.Scale.MajorStep / 5;
            //showCurveResultPanel.YAxis.MajorGrid.IsVisible = true;
            //showCurveResultPanel.YAxis.MinorTic.IsInside = true;
...
//		abstract internal float GetMinPix( GraphPane pane );
		...
		private string MakeTitle()
		{
            ...
            if (_scale._mag != 0 )//&& !_title._isOmitMag && !_scale.IsLog
            {//È¥µô10^3  //return _title._text + String.Format(" (10^{0})", _scale._mag);
                if (_title._text.Contains("¦Ìm"))
                    _title._text =  _title._text.Replace("¦Ìm", "mm");
                if (_title._text.Contains(",N"))
                    _title._text = _title._text.Replace(",N", ",kN");
                return _title._text;
            }
            else
            {
                if (_title._text.Contains("mm"))
                    _title._text = _title._text.Replace("mm", "¦Ìm");
                if (_title._text.Contains(",kN"))
                    _title._text = _title._text.Replace(",kN", ",N");
                return _title._text; 
            }
