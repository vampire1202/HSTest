...
namespace FormPrinting
{
    ...
    public class FormPrinting
    {
        ...
        public bool LabelInBold = false; // Print all labels in bold
        ...
        public string PageNumberingFormat = "µÚ{0}Ò³"; // String format for page number
        ...
        public FormPrinting(System.Windows.Forms.Control f)
        {
            ...
            AddDelegateToPrintControl("ZedGraph.ZedGraphControl", new ControlPrinting(PrintZedGraphControl));
        ...
        private ZedGraph.ZedGraphControl _z;
        ...
        public void PrintToTifFile(string fileName)
        {
            try
            {
                ...
                do
                {
                    ...
                    PrintControls(_f, _z, _MultiPage, _xform, y, out globalExtendedHeight);
        ...
        public void InitPrinting(ref PrintDocument pd)
        {
            ...
            ref Single extendedHeight, out bool ScanForChildControls)
        {
            ...
            Font printFont = new Font(c.Font.Name, (Single)(c.Font.Size * double.Parse(HR_Test.RWconfig.GetAppSettings("reportTitleFontSize"))), FontStyle.Bold);
            float fontHeight = mp.FontHeight(printFont);
            Pen pen = new Pen(Color.Black, 2);
            extendedHeight = fontHeight + 3 + pen.Width + 1;
            mp.BeginPrintUnit(y, extendedHeight);
            mp.DrawString(HR_Test.RWconfig.GetAppSettings("reportTitle"), printFont, Brushes.Black, x, y, c.Width, fontHeight);
            y += fontHeight + 3;
        ...
        public void PrintControls(System.Windows.Forms.Control c, ZedGraph.ZedGraphControl z,
            MultiPageManagement mp,
            Single x, Single y,
            out Single globalExtendedHeight)
        {
            ...
            for (int i = 0; i < nbCtrl; i++)
            {
                ...
                Single extendedHeight;
                PrintControl(controls[i], z, mp, x + controls[i].Location.X, y + cp, out extendedHeight);
                if (extendedHeight > 0)
                {
        ...
        public void PrintControl(System.Windows.Forms.Control c, ZedGraph.ZedGraphControl z,
            MultiPageManagement mp,
            Single x, Single y,
            out Single extendedHeight)
        {
            ...
            if (c.Visible == true)
            {
                ...
                {
                    // Print my header
                    PrintOneControl(c, z, ParentControlPrinting.BeforeChilds, mp, x, y, ref extendedHeight, out scanForChildControls);
                    ...
                    if (scanForChildControls)
                    {
                        ...
                        PrintControls(c, z, mp, x, y, out ChildControlsExtendedHeight);
                        _indent -= 1;

                        // Print my bottom (habitually a frame arround child controls)
                        PrintOneControl(c, z, ParentControlPrinting.AfterChilds, mp, x, y, ref ChildControlsExtendedHeight, out scanForChildControls);
        ...
        /// <param name="y"></param>
        /// <param name="extendedHeight"></param>
        public void PrintOneControl(System.Windows.Forms.Control c, ZedGraph.ZedGraphControl z,
            ParentControlPrinting typePrint,
            MultiPageManagement mp,
            Single x, Single y,
            ref Single extendedHeight, out bool ScanForChildControls)
        {
            ...
            {
                // Silver color if disable
                if (z.Enabled || !DisabledControlsInGray)
                {
                    _Pen = new Pen(Color.Black);
                    _Brush = Brushes.Black;
                }
                            ...
                            HorizontalAlignment hAlignment)
        {
            ...
            mp.DrawString(c.Text, printFont, _Brush, x, yAdjusted, c.Width + c.Font.Size * 2, c.Height + 5, BuildFormat(hAlignment));
        }
            ...
            ref Single extendedHeight, out bool ScanForChildControls)
        {
            ...
            mp.DrawImage(pic.Image, x, y, c.Width, c.Height, GraphicsUnit.Pixel);
            mp.EndPrintUnit();
        }


        public void PrintZedGraphControl(System.Windows.Forms.Control c,
            ParentControlPrinting typePrint,
            MultiPageManagement mp,
            Single x, Single y,
            ref Single extendedHeight, out bool ScanForChildControls)
        {
            ScanForChildControls = false;
            extendedHeight = mp.BeginPrintUnit(y, c.Height);
            ZedGraph.ZedGraphControl zc = (ZedGraph.ZedGraphControl)c;
            mp.DrawZedGraphControl(zc, x, y, zc.Width, zc.Height);
            mp.EndPrintUnit();
        }
        ...
        private void AutoSizeCol(DataGrid dg, int col)
        {
            float width = 0;
            ...
            SizeF headerSize;
            headerSize = g.MeasureString(dg.TableStyles[0].GridColumnStyles[col].HeaderText.ToString(), dg.Font, 400, sf);
            for (int i = 0; i < numRows; ++i)
            {
                size = g.MeasureString(dg[i, col].ToString(), dg.Font, 400, sf);
                if (size.Width > width)
                    width = size.Width;
                if (headerSize.Width > size.Width)
                    width = headerSize.Width;
            }
            dg.TableStyles[0].GridColumnStyles[col].Width = (int)width + 20;
            g.Dispose();
        }
            ...
            ref Single extendedHeight, out bool ScanForChildControls)
        {
            ...
            Font printFont = new Font(dg.Font.Name, dg.Font.Size, FontStyle.Regular);
            Single h = mp.FontHeight(printFont);
            ...
            if (dg.TableStyles.Count == 0)
            {
                ...
                for (int i = 0; i < dt.Columns.Count; i++)
                {
                    ...
                    dgtbox.HeaderText = dt.Columns[i].ColumnName;
                    dgtbox.MappingName = dt.Columns[i].ColumnName;
        ...
        public class MultiPageManagement
        {
            ...
            public void DrawImage(Image image, Single x, Single y, Single w, Single h, GraphicsUnit g)
            {
                if (PrintUnitIsInCurrentPage())
                {
                    Single yPage = _ConvertToPage(y);
                    //_G.DrawImage(image, x, yPage, w, h);
                    _G.DrawImage(image, (int)x, (int)y, new RectangleF(new PointF(0, 0), new Size((int)w, (int)h)), g);
                }
            }

            public void DrawZedGraphControl(ZedGraph.ZedGraphControl z, Single x, Single y, Single w, Single h)
            {
                z.GraphPane.ReSize(_G, new RectangleF(x, y, w, h));
                z.GraphPane.Draw(_G);
            }
