...
{
    public partial class GBT23615TensileZong : Form
    {
        ...
        BLL.GBT236152009_TensileZong m_bllzong;
        Model.GBT236152009_TensileZong m_modelzong;
        private void frmAnalysiseCurve_Load(object sender, EventArgs e)
        {
            ...
            //将计算后的值呈现在Label上
            m_bllzong = new HR_Test.BLL.GBT236152009_TensileZong();
            m_modelzong = m_bllzong.GetModel(_TestSampleNo);
            if (m_modelzong != null)
            {
                if (this.flowLayoutPanel1.Controls.Find("Fmax", false).Length > 0)
                {
                    UC.Result lblFm = (UC.Result)this.flowLayoutPanel1.Controls.Find("Fmax", false)[0];
                    lblFm.Text = m_modelzong.Fmax.ToString();
                }

                if (this.flowLayoutPanel1.Controls.Find("T2", false).Length > 0)
                {
                    UC.Result lblRm = (UC.Result)this.flowLayoutPanel1.Controls.Find("T2", false)[0];
                    lblRm.Text = m_modelzong.T2.ToString();
                }
                if (this.flowLayoutPanel1.Controls.Find("E", false).Length > 0)
                {
                    UC.Result lblFeL = (UC.Result)this.flowLayoutPanel1.Controls.Find("E", false)[0];
                    lblFeL.Text = (m_modelzong.E).ToString();
                }
                if (this.flowLayoutPanel1.Controls.Find("Z", false).Length > 0)
                {
                    UC.Result lblFeZ = (UC.Result)this.flowLayoutPanel1.Controls.Find("Z", false)[0];
                    lblFeZ.Text = (m_modelzong.Z).ToString();
                }

                m_L0 = (double)m_modelzong.L0;
                m_S0 = (double)m_modelzong.S0;
        ...
        private void gBtnSaveResult_Click(object sender, EventArgs e)
        {
            foreach (UC.Result ucR in this.flowLayoutPanel1.Controls)
            {
                ...
                {
                    case "Fmax":
                        m_modelzong.Fmax = double.Parse(ucR.txtFiledContent.Text);
                        break;
                    case "T2":
                        m_modelzong.T2 = double.Parse(ucR.txtFiledContent.Text);
                        break;
                    case "E":
                        m_modelzong.E = double.Parse(ucR.txtFiledContent.Text);
                        break;
                    case "A":
                        m_modelzong.Z = double.Parse(ucR.txtFiledContent.Text);
                        break;
                }
            }
            if (m_bllzong.Update(m_modelzong))
            {
        ...
        private void cmbYr_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            initResultCurve(this.zedGraphControl);
            InitCurve(this.zedGraphControl, this.tslblSampleNo.Text, "GBT23615-2009TensileZong", _lineColor); 
        ...
        void _palZoom_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Right)
            {
                ...
                { 
                    ...
                    double _yl = deltaF / m_S0;
                    double _yb = deltaL /( m_L0*1000.0);

                    if (_yb != 0)
                        m_E = Math.Round(_yl /_yb,0);
                    else
                        m_E = 0;
                    ...
                    //添加 值 的标注
                    ZedGraph.TextObj t = new TextObj("E = " + m_E.ToString("f0") + " MPa", _List_Data[m_secondPIndex].D1, _List_Data[m_secondPIndex].F1);
        ...
        private void tsBtnFp02_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show(this, "请选择 负荷 - 位移 曲线!", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
        ...
        private void gBtnSave_Click(object sender, EventArgs e)
        {
            ...
            if (m_E > 0 && this.flowLayoutPanel1.Controls.Find("E", false).Length > 0)
            {
                UC.Result lblC1 = (UC.Result)this.flowLayoutPanel1.Controls.Find("E", false)[0];
                lblC1.Text = (m_E).ToString("f2");
            }

            if (m_FmIndex > 0 && this.flowLayoutPanel1.Controls.Find("Fmax", false).Length > 0)
            {
                UC.Result lblFTmax = (UC.Result)this.flowLayoutPanel1.Controls.Find("Fmax", false)[0];
                lblFTmax.Text = (_List_Data[m_FmIndex].F1/1000.0).ToString("f2");
            }

            if (m_T > 0 && this.flowLayoutPanel1.Controls.Find("T2", false).Length > 0)
            {
                UC.Result lblFTmax = (UC.Result)this.flowLayoutPanel1.Controls.Find("T2", false)[0];
                lblFTmax.Text = (m_T).ToString("f4");
            }

            if (m_A > 0 && this.flowLayoutPanel1.Controls.Find("A", false).Length > 0)
            {
                UC.Result lbla = (UC.Result)this.flowLayoutPanel1.Controls.Find("A", false)[0];
                lbla.Text = (m_A).ToString("f2");
            } 
        }
        ...
        private void zedGraphControl_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            //    m_HandFeH = Convert.ToDouble((_List_Data[nearP].F1).ToString("f2"));
            //    m_HandReH = Convert.ToDouble((m_HandFeH / m_S0).ToString("G5"));

            //    ZedGraph.TextObj t = new TextObj("FeH = " + m_HandFeH + " N\r\nReH=" + m_HandReH + " MPa", this.zedGraphControl.GraphPane.CurveList[0].Points[nearP].X, this.zedGraphControl.GraphPane.CurveList[0].Points[nearP].Y);
            ...
            ZedGraph.CurveItem ci = null;
            PointF p = (PointF)e.Location;
            if (this.zedGraphControl.GraphPane.FindNearestPoint(p, this.zedGraphControl.GraphPane.CurveList, out ci, out nearP) && m_FlagHandFmax)
            {
                this.zedGraphControl.GraphPane.GraphObjList.RemoveAll(FindAllFeH);
                this.zedGraphControl.GraphPane.CurveList.RemoveAll(FindAllFeH);
                //画点标注
                ...
                m_T = Convert.ToDouble((_List_Data[nearP+1].F1 / m_S0).ToString("f2"));

                ZedGraph.TextObj t = new TextObj("FTmax = " + m_Fm + " kN\r\nT2=" + m_T + " MPa", ci.Points[nearP+1].X,ci.Points[nearP+1].Y);
        ...
        private void tsbtnMax_Click(object sender, EventArgs e)
        {
            ...
            //        //添加 值 的标注
            //        m_T = (_List_Data[m_FmIndex].F1 / 1000.0 * m_L0);
            //        ZedGraph.TextObj t = new TextObj("Fmax = " + (_List_Data[m_FmIndex].F1/1000.0).ToString("f2") + " kN \r\nT2 = " + m_T.ToString("f2")+" MPa", _List_Data[m_FmIndex].D1, _List_Data[m_FmIndex].F1);
            //        t.FontSpec.FontColor = Color.Navy;
            //        t.Location.AlignH = AlignH.Right;
            //        t.Location.AlignV = AlignV.Bottom;
            //        t.FontSpec.IsBold = true;
        ...
        {
            frmAZ az = new frmAZ();
            az._L0 = (double)m_modelzong.L0;
            //az._S0 = (double)m_modelzong.S0;
            if (DialogResult.OK == az.ShowDialog())
            { 
                if (this.flowLayoutPanel1.Controls.Find("A", false).Length > 0)
                {
