...
namespace HR_Test
{
    public partial class frmAnalysiseCurveBend : Form
    {
        ...
        private void frmAnalysiseCurve_Load(object sender, EventArgs e)
        {
            ...
            if (modelBend != null)
            {
                if (this.flowLayoutPanel1.Controls.Find("Fbb", false).Length > 0)
                {
                    UC.Result lblFm = (UC.Result)this.flowLayoutPanel1.Controls.Find("Fbb", false)[0];
                    lblFm.Text = modelBend.Fbb.ToString();
                }
                if (this.flowLayoutPanel1.Controls.Find("σbb", false).Length > 0)
                {
                    UC.Result lblRm = (UC.Result)this.flowLayoutPanel1.Controls.Find("σbb", false)[0];
                    lblRm.Text = modelBend.σbb.ToString();// (m_Fm / m_S0).ToString("G5");
                }
                if (this.flowLayoutPanel1.Controls.Find("Fpb", false).Length > 0)
                {
                    UC.Result lblFeL = (UC.Result)this.flowLayoutPanel1.Controls.Find("Fpb", false)[0];
                    lblFeL.Text = (modelBend.Fpb).ToString();// (_List_Data[m_FRLIndex].F1 / 1000.0).ToString("G5");
                }

                if (this.flowLayoutPanel1.Controls.Find("σpb", false).Length > 0)
                {
                    UC.Result lblFeH = (UC.Result)this.flowLayoutPanel1.Controls.Find("σpb", false)[0];
                    lblFeH.Text = (modelBend.σpb).ToString(); //(_List_Data[m_FRHIndex].F1 / 1000.0).ToString("G5");
                }
                if (this.flowLayoutPanel1.Controls.Find("Eb", false).Length > 0)
                {
                    UC.Result lblReL = (UC.Result)this.flowLayoutPanel1.Controls.Find("Eb", false)[0];
                    ...
                    foreach (Control c in this.flowLayoutPanel1.Controls)
                    {
                        ...
                        switch (ucR.Name)
                        {
                            ...
                            case "Fo(N)":
                                ...
                                break;
                            case "Fpb(kN)":
                                modelBs.Fpb = double.Parse(ucR.txtFiledContent.Text);
                                break;
                            case "Frb(kN)":
                                modelBs.Frb = double.Parse(ucR.txtFiledContent.Text);
                                break;
                            case "Fbb(kN)":
                                modelBs.Fbb = double.Parse(ucR.txtFiledContent.Text);
                                break;
                            case "Fn(kN)":
        ...
        private void tsbtnMax_Click(object sender, EventArgs e)
        {
            if (this.cmbYr.SelectedIndex == 1 & this.cmbXr.SelectedIndex == 2)//负荷-位移
            {
                if (m_FmIndex != 0)
                {
                    ...
                    //添加 值 的标注
                    ZedGraph.TextObj t = new TextObj("Fbb = " + (_List_Data[m_FmIndex].F1 / 1000.0).ToString("G5") + " kN \r\n σbb=" + utils.GetRoundEbOfBend(_List_Data[m_FmIndex].YL1) + " MPa", _List_Data[m_FmIndex].D1, _List_Data[m_FmIndex].F1);
                    t.FontSpec.FontColor = Color.Navy;
                    t.Location.AlignH = AlignH.Right;
                    t.Location.AlignV = AlignV.Bottom;
                    t.FontSpec.IsBold = true;
